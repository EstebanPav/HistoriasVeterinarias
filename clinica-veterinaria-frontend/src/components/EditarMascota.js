...
const EditarMascota = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resMascota = await axios.get(`http://localhost:5000/api/mascotas/${id}`);
        const resPropietarios = await axios.get("http://localhost:5000/api/propietarios");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/editar-mascotas/${id}`, datosActualizados);
