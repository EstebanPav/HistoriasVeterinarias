    ...
    const fetchHistoriasClinicas = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/historia_clinica/${id}`
        );
  ...
  const handleEliminarHistoria = async (historiaId) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:5000/api/historia_clinica/${historiaId}`
      );
      if (response.status === 200) {
        alert("✅ Historia clínica eliminada correctamente.");
        setHistoriasClinicas(
          historiasClinicas.filter(
            (historia) => historia.historia_id !== historiaId
          )
        );
        setFilteredHistorias(
          filteredHistorias.filter(
            (historia) => historia.historia_id !== historiaId
          )
        );
      } else {
    ...
    <div className="dashboard-container">
      ...
      <div className="historia-container">
        ...
        <button
          ...
          onClick={() => navigate(`/registrar-historia/${id}`)}
        >
          ...
          <div className="tabla-container">
                  ...
                  <tr key={historia.historia_id}>
                    <td>
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            `/editar-historia-clinica/${historia.historia_id}`
                          )
                        }
                      >
                        ✏️
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleEliminarHistoria(historia.historia_id)
                        }
                      >
                        🗑
                      </button>
            ...
            <div className="pagination">
              ...
              <div className="pagination-numbers">
                {Array.from(
                  {
                    length: Math.ceil(filteredHistorias.length / itemsPerPage),
                  },
                  (_, index) => (
                    <button
                      key={index + 1}
