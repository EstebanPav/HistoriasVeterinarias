...
import { FaEdit, FaTrash, FaArrowLeft, FaUser, FaFileMedical, FaNotesMedical, FaCalendarAlt } from "react-icons/fa";
...
const MascotaDetallesPage = () => {
    ...
    const handleEliminarMascota = async () => {
        ...
        try {
            await axios.delete(`http://localhost:5000/eliminar/mascotas/${mascota.id}`);
        ...
        <div className="dashboard-container">
            ...
            <div className="details-content">
                ...
                <div className="extra-buttons">
                    <button className="view-owner-btn" onClick={() => navigate(`/ver-propietario/${mascota.id}`)}>
                        <FaUser /> Ver Dueño de Mascota
                    </button>

                    <button className="view-history-btn" onClick={() => navigate(`/ver-historia-clinica/${mascota.id}`)}>
                        <FaFileMedical /> Ver Historia Clínica
                    </button>

                    <button className="view-exam-btn" onClick={() => navigate(`/ver-examen-clinico/${mascota.id}`)}>
                        <FaNotesMedical /> Ver Exámenes Clínicos
                    </button>

                    {/* 📅 Nuevo botón para Ver Calendario */}
                    <button className="view-calendar-btn" onClick={() => navigate("/calendario")}>
